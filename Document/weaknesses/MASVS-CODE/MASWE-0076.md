...
- **ソフトウェア部品表 (SBOM) を使用する**: すべてのコンポーネントと推移的依存関係を追跡するために SBOM を作成して維持し、サードパーティコードの可視性と説明責任を確保します。SBOM とそのソフトウェア依存関係管理の重要性に関する詳細については [NIST SSDF (NIST SP 800-218) PS.3.2](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-218.pdf), [NTIA The Minimum Elements For a Software Bill of Materials (SBOM)](https://www.ntia.doc.gov/files/ntia/publications/sbom_minimum_elements_report.pdf), [CISA SBOM Types document](https://www.cisa.gov/sites/default/files/2023-04/sbom-types-document-508c.pdf) を参照してください。
- **依存関係を責任をもって更新する**: 安全な [依存関係管理](https://cheatsheetseries.owasp.org/cheatsheets/Vulnerable_Dependency_Management_Cheat_Sheet.html) の一環として、使用しているすべての依存関係をセキュリティ関連のアップデートについて (たとえば、CI/CD パイプラインでソフトウェア構成解析 (SCA) ツールや SBOM を使用することによって) 定期的に監視します。既知の脆弱性を修正するアップデートを適用し、明示的にバージョンを固定して、予期しない変更を防ぎ、サプライチェーン攻撃のリスクを軽減します。
- **使用されていない依存関係や廃止した依存関係を削除する**: 未使用、レガシー、または不要なライブラリを定期的にレビューして除去し、アプリの攻撃対象領域と依存関係のフットプリントを削減します。
- **信頼できるソースを使用する**: 公式リポジトリや適切に保守されているオープンソースプロジェクトなど、評判の良いソースからのライブラリや SDK のみを含め、悪意のあるコードや脆弱なコードを持ち込むリスクを最小限に抑えます。
